...
double get_amplitude(double x, double y, int n, int m) {
...
int main(int argc, char** argv) {
    ...
    for (int iter = 0; iter < ITERATIONS; ++iter) {
        ...
        for (auto& g : local_grains) {
            ...
            double next_x = std::clamp(g.x + step_move(gen) * current_amp, 0.0, 1.0);
            double next_y = std::clamp(g.y + step_move(gen) * current_amp, 0.0, 1.0);
        ...
        for (int i = 0; i < size; ++i) send_counts[i] = (int)buckets[i].size();
        ...
        for (auto& b : buckets) send_buf.insert(send_buf.end(), b.begin(), b.end());
        ...
        MPI_Alltoallv(send_buf.data(), send_counts.data(), s_displs.data(), MPI_GRAIN,
                      local_grains.data(), recv_counts.data(), r_displs.data(), MPI_GRAIN, 
                      MPI_COMM_WORLD);
    ...
    if (rank == 0) global_canvas.resize(WIDTH * HEIGHT);

    MPI_Reduce(local_canvas.data(), global_canvas.data(), WIDTH * HEIGHT, 
    ...
    if (rank == 0) {
        ...
        uint32_t max_val = 0;
        for (auto v : global_canvas) if (v > max_val) max_val = v;
